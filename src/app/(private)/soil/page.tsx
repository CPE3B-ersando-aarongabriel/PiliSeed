...
import { ChevronDown, Droplets, FlaskConical, Leaf } from "lucide-react";
...
export default function SoilData() {
			...
			try {
				...
				const farmList =
					...
					typeof (responseBody as { data?: unknown }).data === "object"
						? (((responseBody as { data?: { farms?: FarmOption[] } }).data?.farms ?? []) as FarmOption[])
						: [];
				...
				setSelectedFarmId((currentSelectedFarmId) => {
					...
					return farmList.find((farm) => farm.isActive)?.id ?? farmList[0]?.id ?? "";
				});
			} catch (error) {
				const message =
	...
	const selectedFarmLabel =
		selectedFarm?.name ?? (isLoadingFarms ? "Loading farms..." : "Select a farm");
		...
		<div className="min-h-screen bg-[#EFF6E7]">
			...
			<div className="flex gap-6 p-6">
				<div className="flex-1">
					<SoilInputForm farm={selectedFarm} currentUser={currentUser} />
				</div>
						...
						<div className="flex flex-col gap-6">
							{metrics.map((metric, index) => (
								<div key={index} className="flex items-start gap-4">
									...
									>
										{metric.iconKey === "moisture" && (
											<Droplets className="w-4 h-4 text-[#00450D]" strokeWidth={1.75} aria-hidden="true" />
										)}
										{metric.iconKey === "ph" && (
											<FlaskConical className="w-4 h-4 text-[#795548]" strokeWidth={1.75} aria-hidden="true" />
										)}
										{metric.iconKey === "npk" && (
											<Leaf className="w-4 h-4 text-[#2E577A]" strokeWidth={1.75} aria-hidden="true" />
										)}
									</div>
									<div className="flex-1">
