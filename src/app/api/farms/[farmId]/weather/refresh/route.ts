...
type FarmWeatherRefreshContext = {
...
export async function POST(
  request: Request,
  context: FarmWeatherRefreshContext,
) {
  try {
    ...
    if (!farm.location?.trim()) {
      return errorResponse(
        ...
        "Farm location is required before refreshing weather.",
      );
    }
    ...
    const weather = await weatherService.refreshWeatherSnapshot({
