...
type RecommendationGenerateContext = {
...
function toNormalizedSoilSnapshot(soilProfile: Awaited<ReturnType<typeof getLatestSoilProfileForFarm>>): NormalizedSoilSnapshot | null {
  if (!soilProfile) {
    return null;
  }
...
function buildOpenAIPrompt(input: {
  ...
  userInput: z.infer<typeof generateRecommendationSchema>;
...
async function generateOpenAIRecommendation(input: {
  farmName: string;
  farmLocation: string | null;
  userInput: z.infer<typeof generateRecommendationSchema>;
  soil: NormalizedSoilSnapshot | null;
...
function sanitizeRecommendationRecord(input: {
  ...
  farmName: string;
  farmLocation: string | null;
  inputSnapshot: z.infer<typeof generateRecommendationSchema>;
...
export async function POST(
  request: Request,
  context: RecommendationGenerateContext,
) {
  try {
    ...
    const validationResult = generateRecommendationSchema.safeParse(requestBody);
    ...
    const latestSoilProfile = await getLatestSoilProfileForFarm(decodedToken.uid, farm.id);
