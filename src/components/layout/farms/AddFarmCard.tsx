...
export default function AddFarmCard({
  ...
  async function handleSearch() {
    ...
    try {
      const { response, body } = await fetchWithAuth(currentUser, "/api/location/geocode", {
        method: "POST",
        body: JSON.stringify({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await Promise.resolve(onAdd(farmName, selectedMatch.formattedAddress, selectedMatch));
      setFarmName("");
      setLocation("");
      setMatches([]);
      setSelectedMatch(null);
  ...
  return (
    <>
      <button
        ...
        className="relative w-full sm:w-59 sm:h-59 inline-flex flex-col items-center justify-center px-4 sm:px-5 py-4 bg-[#00450D] rounded-[20px] sm:rounded-[36px] overflow-hidden shadow-[0px_8px_10px_-6px_#0000001a,0px_20px_25px_-5px_#0000001a] cursor-pointer hover:bg-[#005610] transition-colors"
      >
        ...
        <div className="inline-flex items-center pt-1 pb-1.5 px-0 flex-col relative flex-[0_0_auto]">
          <div className="relative flex items-center justify-center h-6 -mt-px font-['Manrope-Bold',Helvetica] font-bold text-white text-base text-center tracking-[0] leading-6">
            Add Farm
          </div>
        </div>

        <div className="inline-flex flex-col max-w-42.5 items-center px-2 py-0 relative flex-[0_0_auto]">
          <div className="relative font-['Inter-Regular',Helvetica] font-normal text-[#ffffffb2] text-xs text-center tracking-[0] leading-4">
            Expand your
            <br />
            digital greenhouse
          </div>
      ...
      {isModalOpen && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 z-50 w-[92vw] max-w-112.5 -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-4 shadow-xl sm:p-6">
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="shrink-0 rounded-xl bg-[#00450D] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#005610] disabled:cursor-not-allowed disabled:opacity-60"
                >
              ...
              <div
                className={`grid transition-[grid-template-rows,opacity] duration-300 ease-out ${matches.length > 0 ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}
              >
                <div className="min-h-0 overflow-hidden">
                  {matches.length > 0 && (
                    ...
                    const isSelected =
                      ...
                      selectedMatch?.formattedAddress === match.formattedAddress;
                    ...
                    return (
                      <button
                        ...
                        className={`w-full rounded-xl border px-4 py-3 text-left transition ${isSelected ? "border-[#00450D] bg-[#EAF4E2]" : "border-[#41493E]/20 bg-white hover:bg-[#F8FAF5]"}`}
                      >
